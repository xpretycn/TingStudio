...
**图表来源**
- [VersionList.vue:71-138](file://frontend/src/views/versions/VersionList.vue#L71-L138)
- [version.ts:6-88](file://frontend/src/stores/version.ts#L6-L88)
- [version.ts:18-34](file://frontend/src/api/version.ts#L18-L34)

#### 版本对比组件

**更新** 版本对比组件实现了更加直观的差异可视化，包含字段类型分类和变更类型标识：
...
User->>CompareView : 选择两个版本
CompareView->>Store : 调用compareVersions()
Store->>API : 调用compare接口
API->>Controller : 发送对比请求
Controller->>Controller : 加载两个版本数据
Controller->>Controller : 执行差异比较
Controller->>Controller : 比较配方字段
Controller->>Controller : 比较参数变更
Controller->>Controller : 比较原料列表
Controller->>Controller : 计算统计信息
Controller-->>API : 返回对比结果
API-->>Store : 返回结果
Store-->>CompareView : 更新状态
CompareView-->>User : 显示对比结果
```

**图表来源**
- [VersionCompare.vue:97-135](file://frontend/src/views/versions/VersionCompare.vue#L97-L135)
- [version.ts:61-88](file://frontend/src/stores/version.ts#L61-L88)

**章节来源**
- [VersionList.vue:1-173](file://frontend/src/views/versions/VersionList.vue#L1-L173)
- [VersionCompare.vue:1-165](file://frontend/src/views/versions/VersionCompare.vue#L1-L165)
- [version.ts:1-88](file://frontend/src/stores/version.ts#L1-L88)
...
// 批量删除版本
DELETE /api/versions/batch-delete

// 批量归档版本
PUT /api/versions/batch-archive
```
