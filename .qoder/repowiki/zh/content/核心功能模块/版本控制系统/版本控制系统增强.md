...
- 版本列表表格支持状态筛选、自定义 checkbox 对比选择、差异高亮对比卡片
- 跨页面数据传递通过 localStorage 持久化实现
- TDesign Dialog 替代浏览器原生 confirm 弹窗
...
- **Main**：
  ...
  - 对比卡片网格（横向滚动 flex 布局）
    - 每张卡片展示：版本号 pill、版本名称、日期作者、原料构成进度条、差异高亮、版本摘要
    - 差异类型：diff-added(绿)/diff-changed(黄)/diff-removed(红删除线)
