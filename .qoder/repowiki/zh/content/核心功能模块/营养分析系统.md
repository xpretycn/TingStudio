...
- 前端采用组件化架构，NutritionAnalysis.vue 负责配方营养分析界面，NutritionProfiles.vue 负责营养标准管理，Pinia store 统一管理状态与 API 调用。
...
- [frontend/src/views/nutrition/NutritionAnalysis.vue:1-262](file://frontend/src/views/nutrition/NutritionAnalysis.vue#L1-L262)
...
- 前端视图：NutritionAnalysis.vue 提供配方分析与合规检查界面；NutritionProfiles.vue 提供营养标准创建与查看。
- 状态管理：Pinia store 统一调度 API 请求与状态缓存。
- 数据库：SQLite 存储原料营养、配方汇总、营养标准与分析报告。

**章节来源**
- [backend/src/controllers/nutritionController.ts:55-407](file://backend/src/controllers/nutritionController.ts#L55-L407)
- [frontend/src/views/nutrition/NutritionAnalysis.vue:10-217](file://frontend/src/views/nutrition/NutritionAnalysis.vue#L10-L217)
- [frontend/src/views/nutrition/NutritionProfiles.vue:1-262](file://frontend/src/views/nutrition/NutritionProfiles.vue#L1-L262)
- [frontend/src/stores/nutrition.ts:1-100](file://frontend/src/stores/nutrition.ts#L1-L100)
...
**图表来源**
- [backend/src/routes/nutrition.ts:13-31](file://backend/src/routes/nutrition.ts#L13-L31)
- [backend/src/controllers/nutritionController.ts:55-407](file://backend/src/controllers/nutritionController.ts#L55-L407)
- [frontend/src/api/nutrition.ts:15-37](file://frontend/src/api/nutrition.ts#L15-L37)
- [frontend/src/stores/nutrition.ts:38-84](file://frontend/src/stores/nutrition.ts#L38-L84)
...
**图表来源**
- [backend/src/routes/nutrition.ts:30-30](file://backend/src/routes/nutrition.ts#L30-L30)
...
**章节来源**
- [backend/src/controllers/nutritionController.ts:244-407](file://backend/src/controllers/nutritionController.ts#L244-L407)
- [backend/src/scripts/init.sql:200-226](file://backend/src/scripts/init.sql#L200-L226)
...
B --> C["计算含量比"]
C --> D["逐列累加得到每100g营养值"]
D --> E["能量重新计算仅用处理后三大产能营养素"]
E --> F["生成标签行含 NRV%、阈值、容差"]
F --> G["返回完整表格数据"]
```
...
## 结论
本系统通过标准化的数据模型、严谨的计算流程与完善的数据库设计，实现了从原料到配方的营养分析闭环。前后端职责清晰，API 文档完备，具备良好的可维护性与扩展性。建议持续完善测试用例与性能监控，确保在高并发场景下的稳定性与准确性。
...
**章节来源**
- [backend/API_DOC.md:561-701](file://backend/API_DOC.md#L561-L701)