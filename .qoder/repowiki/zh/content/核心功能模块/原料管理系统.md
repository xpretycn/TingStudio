...
## 更新摘要
**所做更改**
- 更新了数据模型章节，反映了 ratio_factor 字段的数据库结构重构
- 修改了核心业务实体表格，移除了 ratio_factor 字段
- 更新了数据库架构图，反映最新的表结构
- 增加了关于 ratio_factor 字段迁移的技术说明
- 更新了前端组件实现，移除了 ratio_factor 相关的显示逻辑
...
**更新** 系统现已完成数据库结构重构，ratio_factor 字段已从 materials 表迁移至 formulas 表，简化了原料管理功能。
...
- [MaterialList.vue:1-341](file://frontend/src/views/materials/MaterialList.vue#L1-L341)
...
系统采用 SQLite 作为数据存储，核心数据模型围绕原料管理展开。经过数据库结构重构后，ratio_factor 字段已从 materials 表迁移至 formulas 表：
...
**更新** ratio_factor 字段已从 materials 表移除，现在存在于 formulas 表中，用于配方级别的含量比计算。
...
- [materials.ts:11-21](file://backend/src/routes/materials.ts#L11-L21)
...
**章节来源**
- [MaterialList.vue:1-341](file://frontend/src/views/materials/MaterialList.vue#L1-L341)
- [MaterialForm.vue:1-189](file://frontend/src/views/materials/MaterialForm.vue#L1-L189)
...
**图表来源**
- [MaterialList.vue:75-82](file://frontend/src/views/materials/MaterialList.vue#L75-L82)
- [materialController.ts:1-129](file://backend/src/controllers/materialController.ts#L1-L129)
- [materials.ts:1-22](file://backend/src/routes/materials.ts#L1-L22)
...
**更新** 系统已完成关键的数据库结构重构，将 ratio_factor 字段从 materials 表迁移至 formulas 表，这一变化带来了以下改进：

- **简化数据模型**：移除了原料表中的 ratio_factor 字段，使数据结构更加简洁
- **优化业务逻辑**：含量比系数现在与配方关联，符合实际业务场景
- **提升查询性能**：减少了不必要的字段查询，优化了数据库访问
- **增强数据一致性**：通过配方级别的 ratio_factor 管理，确保数据准确性
