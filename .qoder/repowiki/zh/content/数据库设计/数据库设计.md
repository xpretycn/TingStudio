...
- [init.sql:1-227](file://backend/src/scripts/init.sql#L1-L227)
...
- [formulaController.ts:1-268](file://backend/src/controllers/formulaController.ts#L1-L268)
...
#### 原料表 materials
- 主键：id（TEXT，PRIMARY KEY）
- 约束：name、code 唯一（UNIQUE），unit 默认 g，stock 默认 0
...
- 约束：materials_json 为 JSON，finished_weight/ratio_factor 默认值，created_by 引用 users
...
章节来源
- [init.sql:31-48](file://backend/src/scripts/init.sql#L31-L48)
...
章节来源
- [init.sql:54-69](file://backend/src/scripts/init.sql#L54-L69)
- [DATABASE_DOC.md:98-119](file://backend/DATABASE_DOC.md#L98-L119)

### 版本控制
- 表：formula_versions
- 外键：formula_id → formulas(id)（ON DELETE CASCADE）
- 约束：status 枚举（draft/published/archived），is_current 0/1，version_number 唯一性（与 formula_id 组合索引）
- 索引：idx_fv_formula、idx_fv_version_number
- JSON 结构：changes_json（变更记录）、snapshot_json（完整配方快照）

章节来源
- [init.sql:75-90](file://backend/src/scripts/init.sql#L75-L90)
- [DATABASE_DOC.md:122-169](file://backend/DATABASE_DOC.md#L122-L169)
- [formulaController.ts:112-199](file://backend/src/controllers/formulaController.ts#L112-L199)
...
章节来源
- [init.sql:96-165](file://backend/src/scripts/init.sql#L96-L165)
...
章节来源
- [init.sql:171-226](file://backend/src/scripts/init.sql#L171-L226)
...
- [init.sql:28-90](file://backend/src/scripts/init.sql#L28-L90)
...
## 结论
TingStudio 的数据库设计以 SQLite 为核心，围绕配方研发与营养分析构建了清晰的功能域划分与强外键约束体系。通过初始化脚本与种子数据实现了快速落地，配合控制器层的 JSON 字段处理与版本控制机制，满足中小规模业务的日常运营需求。建议在高并发与复杂查询场景下逐步引入规范化表与索引优化，持续提升系统稳定性与性能。
...
- formulas：30 条（覆盖婴幼儿到特殊医学配方）
- formula_versions：30 条（每配方 1 个版本）
...
章节来源
- [init.sql:28-226](file://backend/src/scripts/init.sql#L28-L226)
