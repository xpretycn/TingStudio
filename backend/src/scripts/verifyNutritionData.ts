...
const results = (await query(`
  ...
  LIMIT 20
`, [])).rows as Array<{name: string; id: string; per_100g_json: string; data_source: string}>;
...
if (results.length > 0) {
  ...
  const totalWithNutrition = (await query(`
    SELECT COUNT(*) as count FROM materials m 
    JOIN material_nutrition mn ON m.id = mn.material_id AND mn.is_latest = 1
    WHERE mn.per_100g_json IS NOT NULL AND mn.per_100g_json != '{}'
  `).get() as { count: number };
  
  const totalMaterials = (await query(`SELECT COUNT(*) as count FROM materials`, [])).rows[0] as { count: number };
...
`, [])).rows as Array<{name: string; id: string}>;
