...
async function createMySQLDatabase() {
  ...
  try {
    ...
    await connection.query('CREATE DATABASE IF NOT EXISTS `tingstudio` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');
    console.log('✅ 数据库 "tingstudio" 创建成功');
    
    // 使用新创建的数据库
    await connection.query('USE tingstudio');
    console.log('✅ 已切换到数据库 "tingstudio"');
    
    // 验证数据库
    const [databases] = await connection.query('SHOW DATABASES LIKE "tingstudio"');
