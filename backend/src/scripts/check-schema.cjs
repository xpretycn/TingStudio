...
const sampleFormulas = db.prepare("SELECT id, name, ratio_factor FROM formulas LIMIT 3").all();
console.log('配方示例:');
sampleFormulas.forEach(f => {
  console.log(`  ${f.id}: ${f.name} (ratio_factor: ${f.ratio_factor})`);
});

const sampleVersions = db.prepare("SELECT version_id, formula_id, ratio_factor FROM formula_versions LIMIT 3").all();
console.log('版本示例:');
sampleVersions.forEach(v => {
  console.log(`  ${v.version_id}: formula ${v.formula_id} (ratio_factor: ${v.ratio_factor})`);
});
