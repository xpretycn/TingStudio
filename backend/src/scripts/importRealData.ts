...
async function importData() {
  ...
  const importMaterials = db.transaction(() => {
    for (const [name, mat] of allMaterials) {
      ...
      // 查询单价（从价格映射表匹配）
      const unitPrice = MATERIAL_PRICE_MAP[name] ?? null;

      db.prepare(
        `
        INSERT INTO materials (id, name, code, unit, stock, material_type, unit_price, data_source, created_by, created_at, updated_at)
        VALUES (?, ?, ?, 'g', 0, ?, ?, 'batch_import', ?, ?, ?)
      `,
      ).run(materialId, name, code, materialType, unitPrice, adminId, timestamp, timestamp);
      ...
      `,
      ).run(nutritionId, materialId, JSON.stringify(per100g), `来源配方: ${mat.sourceFormula}`, timestamp);

      console.log(
        `  ✓ ${name} (${code}) - ${materialType === "supplement" ? "辅料" : "药材"} - 能量: ${energy} kJ/100g`,
      );
    }
  });
