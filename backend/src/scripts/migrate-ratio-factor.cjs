...
async function migrate() {
  try {
    ...
    // 检查是否已经迁移过
    const tableInfo = all("SELECT name FROM pragma_table_info('formulas') WHERE name = 'ratio_factor'");
    ...
    // 获取所有配方及其原料信息
    const formulas = all('SELECT id, materials_json FROM formulas');
    ...
    // 验证 formulas 表
    const formulaCheck = all('SELECT COUNT(*) as count FROM formulas');
    const formulaWithRatio = all('SELECT COUNT(*) as count FROM formulas WHERE ratio_factor IS NOT NULL');
    console.log(`   formulas 表: 总计 ${formulaCheck[0].count} 条记录，其中 ${formulaWithRatio[0].count} 条包含 ratio_factor`);
    
    // 验证 formula_versions 表
    const versionCheck = all('SELECT COUNT(*) as count FROM formula_versions');
    const versionWithRatio = all('SELECT COUNT(*) as count FROM formula_versions WHERE ratio_factor IS NOT NULL');
