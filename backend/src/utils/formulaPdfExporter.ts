...
export async function exportFormulaToPdf(
  ...
  // 列顺序：序号、原料名称、数量(g)、类型、单位
  const colWidths = [40, 200, 90, 80, 85]
  const colHeaders = ['序号', '原料名称', '数量(g)', '类型', '单位']
  const rowHeight = 24
  ...
  const dataCellPadding = 6  // 垂直居中偏移量
  materials.forEach((m: any, i: number) => {
    ...
    // 检查是否需要分页（页面底部预留空间：表尾50px + 营养表头约100px）
    if (y > 700) {
      // 添加新页面
      doc.addPage()
      y = 50  // 重置y坐标
    ...
    const rowData = [String(i + 1), name, qty, type, unit]
    doc.fillColor('#333333')
    x = startX
    for (let j = 0; j < rowData.length; j++) {
      // 所有列都居中对齐
      doc.text(rowData[j], x + 4, y + dataCellPadding, { width: colWidths[j] - 8, align: 'center' })
      x += colWidths[j]
    }
    y += dataRowHeight
  })
