...
export async function getMaterials(req: any, res: Response) {
  try {
    ...
    const [list]: any[] = await query(
      `SELECT * FROM materials ${whereSql} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
      [...params, size, offset]
    )
...
export async function createMaterial(req: any, res: Response) {
  try {
    const { name, code, unit, stock } = req.body
    const userId = req.user.userId
    const id = generateId()

    await query(
      `INSERT INTO materials (id, name, code, unit, stock, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [id, name, code, unit || 'g', stock || 0, userId, now()]
    )
...
export async function updateMaterial(req: Request, res: Response) {
  try {
    ...
    await query(
      'UPDATE materials SET name=?, code=?, unit=?, stock=? WHERE id=?',
      [name, code, unit, stock, id]
    )
...
export async function deleteMaterial(req: Request, res: Response) {
  try {
    ...
    const [[usageResult]]: any[][] = await query(
      `SELECT COUNT(*) as cnt FROM formulas WHERE materials_json LIKE ?`,
      [`%"materialId":"${id}"%`]
    )
