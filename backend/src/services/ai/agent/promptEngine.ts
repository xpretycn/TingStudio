...
- **系数(ratio_factor)**：药材默认0.18，辅料默认1.0。含量比 = Σ(用量×系数)/成品重量，正常范围0.95~1.05
- **成品重量**：单位为克(g)，通常为正整数，范围50~5000g
- **原料类型**：herb=药材（需提取），supplement=辅料（直接添加）
- **原料单位**：默认g(克)，单价单位为元/公斤
- **含量比校验4级判定**：normal(0.95~1.05) → warning(0.85~0.95或1.05~1.15) → high_warning(0.75~0.85或1.15~1.25) → error(<0.75或>1.25)
- **营养标签NRV%**：基于中国食品营养标签参考值计算
- **配方编码规则**：自动生成，格式为FM+年月日+序号
- **业务员状态**：active=在职，inactive=离职
- **配方命名规范**：通常以主要药材或功效命名，如"佛手玫苓膏"、"健脾养胃膏"`,
  version: "4.0.0",
