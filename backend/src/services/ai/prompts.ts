...
可检索的数据范围：**配方(formulas)**、**原料(materials)**、**业务员(salesmen)**、**配方销量(formula_sales)**、**配方版本(formula_versions)**。
...
- packaging_price REAL 包装价格
- other_price REAL 其他费用
- profit_margin REAL 利润率(%)
- description TEXT 描述
- preparation_method TEXT 制法
- original_name TEXT 原配方名称
- original_weight REAL 原配方重量
...
- unit TEXT 单位（默认 g）
- stock REAL 库存数量
- material_type TEXT 类型（herb=药材/主料, supplement=辅料）
- unit_price REAL 单价（元/kg）
- data_source TEXT 数据来源（manual/imported）
- created_by TEXT 创建者ID
- created_at TEXT 创建时间
- version INTEGER 版本号
- is_latest INTEGER 是否最新版本（1=是, 0=否）
- is_deleted INTEGER 是否已删除（1=已删除, 0=正常）
- status TEXT 状态（draft/pending_review/published）
...
- phone TEXT 电话
- email TEXT 邮箱
- status TEXT 状态（active=在职, inactive=离职）
- created_by TEXT 创建者ID
- created_at TEXT 创建时间
...
- version_reason TEXT 升版原因
- changes_json TEXT 变更内容JSON
- snapshot_json TEXT 快照JSON
- status TEXT 状态（draft/published/archived）
- is_current INTEGER 是否当前版本
- ratio_factor REAL 系数
- supplement_ratio_factor REAL 补充系数
- created_by TEXT 创建者ID
- created_at TEXT 创建时间

### 表：formula_sales（配方销量表）
- id TEXT PRIMARY KEY
- formula_id TEXT 配方ID
- salesman_id TEXT 业务员ID
- period_type TEXT 周期类型（monthly/quarterly/yearly）
...
- per_100g_json TEXT 每100g营养成分JSON，格式：{"protein":数值,"fat":数值,"carbohydrate":数值,"sodium":数值}
- data_version TEXT 数据版本
- data_source TEXT 数据来源
- confidence TEXT 置信度（high/medium/low）
- last_updated TEXT 更新时间
- material_version INTEGER 原料版本号
- is_latest INTEGER 是否最新

### 表：nutrition_profiles（营养标准档案表）
- profile_id TEXT PRIMARY KEY
- name TEXT 档案名称
- description TEXT 描述
- category TEXT 分类（infant/child/adult/elderly/pregnant/special）
- target_values_json TEXT 目标值JSON
- is_preset INTEGER 是否预设

注意事项：
1. SQLite 日期比较使用 strftime('%Y-%m-%d', created_at) = '2026-01-01' 格式
...
5. 查询原料时请过滤 is_deleted = 0，只查询未删除的原料
6. 查询最新版本的原料时请加 is_latest = 1
7. 查询结果列名使用中文别名，方便用户阅读
`;
...
2. 只允许查询以下表：formulas, materials, salesmen, formula_versions, material_nutrition, formula_sales, nutrition_profiles
3. 结果限制 50 条，除非用户明确要求更多
4. 只返回一条 SQL 语句，不要包含任何解释文字
5. SQL 语句使用 SQLite 语法
6. 查询原料时必须添加 WHERE materials.is_deleted = 0 过滤已删除的原料
7. 查询最新原料时必须添加 WHERE materials.is_latest = 1
...
## 示例查询
- 查询所有配方：
  SELECT id, code, name AS 配方名称, salesman_name AS 业务员, finished_weight AS 成品重量, created_at AS 创建时间 FROM formulas ORDER BY created_at DESC LIMIT 50
- 查询所有原料（未删除）：
  SELECT id, code, name AS 原料名称, material_type AS 类型, stock AS 库存, unit_price AS 单价 FROM materials WHERE is_deleted = 0 ORDER BY name ASC LIMIT 50
- 查询在职业务员：
  SELECT id, name AS 姓名, department AS 部门, phone AS 电话 FROM salesmen WHERE status = 'active' ORDER BY name ASC LIMIT 50
- 按业务员统计配方数量：
...
- 按月统计配方创建趋势：
  SELECT strftime('%Y-%m', f.created_at) AS 月份, COUNT(f.id) AS 配方数量 FROM formulas f GROUP BY 月份 ORDER BY 月份 DESC LIMIT 50
- 库存不足的原料：
  SELECT name AS 原料名称, stock AS 库存, material_type AS 类型 FROM materials WHERE stock < 50 AND is_deleted = 0 ORDER BY stock ASC LIMIT 50
- 查询业务员及其负责的配方：
  SELECT s.name AS 业务员, COUNT(f.id) AS 配方数, GROUP_CONCAT(DISTINCT f.name, '，') AS 配方列表 FROM salesmen s LEFT JOIN formulas f ON s.id = f.salesman_id WHERE s.status = 'active' GROUP BY s.name ORDER BY 配方数 DESC LIMIT 50`;

export const NL2SQL_USER_PROMPT = (userQuery: string) =>
  `用户查询：${userQuery}

请根据用户的自然语言查询，生成对应的 SQLite SELECT 查询语句。

注意：
- 查询原料表时务必添加 is_deleted = 0 过滤条件
- 查询结果列名请使用中文别名（如：SELECT name AS 原料名称）
- 只返回一条 SQL 语句，不要加任何解释文字、代码块标记或引号：
`;
