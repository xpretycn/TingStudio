...
// ─── 品牌色色板映射 ───
export const brandColorPalettes: Record<BrandColor, {
  primary: string
  primaryLight: string
  ...
  green: {
    primary: '#5CB85C',
    primaryLight: '#7DD87D',
    primaryLighter: '#A8E6A8',
    primaryLightest: '#D4F5D4',
    primaryBg: '#EFF8EF',
    primaryDark: '#4A9A4A',
  },
}
...
// 暗色模式下品牌色需要调亮
const darkBrandPalettes: Record<BrandColor, {
  primary: string
  primaryLight: string
  primaryLighter: string
  primaryLightest: string
  ...
  green: {
    primary: '#7DD87D',
    primaryLight: '#A8E6A8',
    primaryLighter: '#C8F0C8',
    primaryLightest: '#DEFFDE',
    primaryBg: '#152515',
    primaryDark: '#5CB85C',
  },
}
...
export function getThemeTokens(isDark: boolean, brand: BrandColor = 'pink') {
  ...
  const states = isDark ? { ...{ colorSuccess, colorWarning, colorDanger }, ...darkStates } : { colorSuccess, colorWarning, colorDanger, colorInfo }
