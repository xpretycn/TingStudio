...
export const useWeatherStore = defineStore('weather', () => {
  ...
  const weatherIcon = computed(() => {
    ...
    if (['阵雨', '小雨', '中雨', '大雨', '暴雨', '毛毛雨', '细雨', '小-中雨', '中-大雨', '大-暴雨'].includes(text)) return 'rainy'
    if (['阵雪', '小雪', '中雪', '大雪', '暴雪', '雨夹雪', '小-中雪', '中-大雪', '大-暴雪'].includes(text)) return 'snowy'
  ...
  async function autoLocate() {
    ...
    try {
      ...
      const reason = err.name === 'AbortError' || err.message?.includes('timeout')
        ? '定位请求超时'
        : `定位失败(${err.message})`
  ...
  async function locateByAmap(): Promise<boolean> {
    ...
    const url = isDev
      ? `/amap/v3/ip?key=${AMAP_KEY}&type=4`
    ...
    if (!searchKeyword) { console.warn('[Weather] 无法提取城市名'); return false }

    console.log('[Weather] 高德城市:', searchKeyword)

    const cities = await searchCity(searchKeyword)
    if (cities.length === 0) { console.warn('[Weather] 城市搜索无结果'); return false }
  ...
  async function locateByIpApi(): Promise<boolean> {
    // ip-api.com 支持 CORS，可直接访问；通过 Vite 代理更稳定
    const url = '/ip-api/json'  // Vite 代理到 http://ip-api.com/json
    console.log('[Weather] ip-api 请求:', url)
    ...
    try {
      ...
      // ip-api 返回：lat, lon, city, regionName, country
      const lat = data.lat
      const lon = data.lon
      ...
      if (searchKeyword) {
        ...
        // 搜索不到则用 ip-api 的坐标做逆地理编码
        console.log('[Weather] ip-api 城市搜索无结果，尝试逆地理编码')
      }

      // 用经纬度做逆地理编码获取 adcode，再用 adcode 查天气
      ...
      console.warn('[Weather] ip-api 失败:', err.message)
      return false
    }
  }
