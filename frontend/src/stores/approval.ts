...
/** 已审核 tab 的排序字段映射：createdAt → reviewedAt */
function resolveAdminSortBy(field: FetchListParams["sortBy"], isHistoryTab: boolean): FetchListParams["sortBy"] {
  if (!field) return undefined;
  // 原料列表：formulaName 映射为 name
  if (field === "formulaName") return "name";
  // 已审核 tab：时间字段用 reviewedAt
  if (isHistoryTab && field === "createdAt") return "reviewedAt";
  return field;
}

/** 我的列表排序字段映射 */
function resolveMySortBy(field: FetchListParams["sortBy"]): FetchListParams["sortBy"] | undefined {
  if (!field) return undefined;
  if (field === "formulaName") return "name";
  // status 排序由前端内存处理，不传给后端
...
export const useApprovalStore = defineStore("approval", () => {
  ...
  const myMaterialStatusCounts = ref<Record<string, number>>({
    draft: 0,
    pending_review: 0,
    published: 0,
    rejected: 0,
  ...
  async function fetchPendingReviews(params?: FetchListParams) {
    ...
    try {
      ...
      const data = await approvalApi.getPendingReviews({
        ...
        pageSize,
        keyword: params?.keyword,
        sortBy: params?.sortBy ?? resolveAdminSortBy(adminSortBy.value, false),
  ...
  async function fetchReviewedHistory(params?: FetchListParams) {
    ...
    try {
      ...
      const data = await approvalApi.getReviewedHistory({
        ...
        keyword: params?.keyword,
        action,
        sortBy: params?.sortBy ?? resolveAdminSortBy(adminSortBy.value, true),
        sortOrder: params?.sortOrder ?? adminSortOrder.value,
        dateRange: params?.dateRange || adminDateRange.value || undefined,
      });
  ...
  async function fetchMyMaterialSubmissions(params?: FetchListParams) {
    ...
    try {
      ...
      const data = await approvalApi.getMyMaterialSubmissions({
        ...
        keyword: params?.keyword,
        status: params?.status,
        sortBy: params?.sortBy ?? resolveMySortBy(myListSortBy.value),
        sortOrder: params?.sortOrder ?? myListSortOrder.value,
        dateRange: params?.dateRange || myListDateRange.value || undefined,
        materialType: params?.materialType || (myListMaterialType.value ? myListMaterialType.value : undefined),
      });
