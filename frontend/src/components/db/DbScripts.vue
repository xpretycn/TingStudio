<template>
  <div class="db-scripts">
    ...
    <t-popconfirm
      v-model:visible="highDangerPopVisible"
      :content="getHighDangerContent()"
      theme="danger"
    ...
    <t-drawer
      ...
      :header="previewScript?.name || '脚本预览'"
      size="720px"
    >
    ...
    <t-drawer
      ...
      :header="historyScript?.name ? `${historyScript.name} - 执行历史` : '执行历史'"
      size="640px"
      :footer="false"
    >
    ...
    <t-drawer
      ...
      :header="versionScript?.name ? `${versionScript.name} - 版本历史` : '版本历史'"
      size="680px"
      :footer="false"
    >
...
async function handleRestoreVersion(ver: ScriptVersion) {
  ...
  try {
    ...
    // If preview drawer is open for this script, refresh its content too
    if (previewDrawerVisible.value && previewScript.value?.id === versionScript.value.id) {
