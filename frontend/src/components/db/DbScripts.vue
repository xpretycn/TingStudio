<template>
  <div class="db-scripts">
    <t-loading :loading="loading && categories.length === 0">
      ...
      <t-empty v-if="!loading && !categories.length" description="暂无可用脚本" />
    </t-loading>
      ...
      <template v-if="detailScript">
        ...
        <div v-if="detailScript.lastExecutedAt" class="drawer-section">
          ...
          <div class="last-exec-info">
            ...
            <t-tag
              :theme="detailScript.lastStatus === 'success' ? 'success' : 'danger'"
              variant="light"
              size="small"
            >
              {{ detailScript.lastStatus === 'success' ? '成功' : '失败' }}
            </t-tag>
          </div>
        </div>
      </template>
...
async function fetchScriptList() {
  loading.value = true
  try {
    const scripts = await http.get<unknown, ScriptDefinition[]>("/db/scripts")
