...
配方营养成分计算**必须**严格遵循以下步骤，任何涉及营养计算的代码（前端展示、后端计算、AI 生成）都不得偏离：

#### 计算步骤

1. **获取配方数据**，区分药材（herb）与辅料（supplement）类型
2. **计算含量比**：`ratio = (quantity / finishedWeight) × ratioFactor`
   - 药材使用 `ratio_factor`（默认 **0.18**）
   - 辅料使用 `supplement_ratio_factor`（默认 **1.0**）
3. **汇总营养**：`Σ(per100g营养素 × 含量比)`
4. **能量计算**（Atwater 系数）：`能量 = 蛋白质 × 17 + 脂肪 × 37 + 碳水化合物 × 17`
5. **NRV% 计算**：`NRV% = (营养素值 / NRV参考值) × 100`
6. **0 界限值归零**（国标要求，必须执行）：
...
| 蛋白质 | 0.5 g | ≤ 0.5 g → 归零 |
| 脂肪 | 0.5 g | ≤ 0.5 g → 归零 |
| 碳水化合物 | 0.5 g | ≤ 0.5 g → 归零 |
| 钠 | 5 mg | ≤ 5 mg → 归零 |

7. **归零后能量重新计算**：若蛋白质/脂肪/碳水化合物被归零，必须用归零后的值重新计算能量（步骤 4），再对能量做 17 kJ 界限判断

#### 关键参数

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `ratio_factor` | 0.18 | 药材折算系数（可由营养档案配置） |
| `supplement_ratio_factor` | 1.0 | 辅料折算系数（可由营养档案配置） |
| `finishedWeight` | 配方成品重量 | 用于计算含量比的分母 |

#### 禁止行为

- **禁止**跳过 0 界限值归零步骤，即使计算结果为极小正值也必须归零
- **禁止**在归零后不重新计算能量
- **禁止**将 Atwater 系数硬编码为其他值（蛋白质/碳水 17，脂肪 37）
- **禁止**混淆药材和辅料的 ratio_factor
