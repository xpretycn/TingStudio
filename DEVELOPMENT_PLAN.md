...
| 认证（登录/注册）  | ✅ 完整                   | ✅ 完整                        | ✅    | 100%   |
| 配方管理 CRUD      | ✅ 完整                   | ✅ 完整                        | ✅    | 100%   |
| 原料管理 CRUD      | ✅ 完整                   | ✅ 列表/详情/表单全面重构      | ✅    | 100%   |
| 业务员管理 CRUD    | ✅ 完整                   | ✅ 完整                        | ✅    | 100%   |
| 版本管理           | ✅ 完整                   | ✅ 完整                        | ✅    | 100%   |
| 导出中心           | ✅ 完整                   | ✅ 完整                        | ✅    | 100%   |
| 营养分析           | ✅ 完整                   | ✅ 完整                        | ✅    | 100%   |
| 营养标准           | ✅ 完整                   | ✅ 完整                        | ✅    | 100%   |
| AI 智能助手        | ✅ 完整（含原料营养解析） | ✅ 完整                        | ✅    | 100%   |
| 账号设置           | -                         | ✅ 完整                        | -     | 100%   |
| 首页/主布局        | -                         | ✅ 完整                        | -     | 100%   |
| 工具箱             | -                         | ⚠️ 天气卡片                    | -     | 30%    |
| 天气服务           | -                         | ✅ 完整（侧边栏 + 工具箱）     | ✅    | 100%   |
...
| 原料模块 UI 重构   | -                         | ✅ 列表/详情/表单 + AI营养解析 | -     | 100%   |
...
### ✅ 阶段十五：原料管理模块全面重构 + AI 营养解析（v2.18.0）

> 完成日期：2026-04-17

#### 15.1 原料列表页重构（MaterialList.vue）

- 数据看板区域：4 个统计卡片（原料总数/药材数量/辅料数量/平均库存）
- 批量操作栏：默认隐藏，选中行后显示批量删除
- 表格展开行：完整营养成分数据（6 项核心指标 + 进度条 + NRV 百分比）
- 活动时间线：近期原料变更记录（创建/编辑/营养更新）
- 分页控件：内置 pageSize=10，操作列标题与内容完美居中对齐
- 原料类型 Tag 区分（药材 emerald / 辅料 amber）

#### 15.2 原料详情页重构（MaterialDetail.vue）

- `header + main` 双区域布局（参照 FormulaDetail.vue）
- Header 含返回按钮 + 面包屑 + 标题 + 操作按钮
- Main 左栏：原料概况 + 变更记录时间线
- Main 右栏：营养成分网格（每100g含量分析 + 能量计算 + 标准参考 Tag）
- 能量计算公式可视化展示

#### 15.3 原料表单页重构（MaterialForm.vue）

- Header 面包屑导航 + 标题 + 保存/取消按钮
- Main 左侧双 Section：基础信息 + 营养成分分组折叠面板
- Main 右侧 AI 智能营养解析面板（新建可用，编辑遮罩禁用）
- 能量字段改为自动计算值（蛋白质×17+脂肪×37+碳水化合物×17）
- 新建模式原料编码自动生成填充

#### 15.4 AI 原料营养解析功能

- 独立新接口 `POST /api/ai/parse-material-nutrition`
- 专用提示词：解析原料名称 + 4 项核心营养素（蛋白/脂肪/碳水/钠）
- 解析结果表格：原料名称 | 营养数据 | 匹配状态 | **操作列**
- 解析可信度概览 + Token 消耗显示
- 「立即录入」按钮：loading → 成功 → 失败状态流转
- 自动完成编码生成、单位默认值、数据来源标记

#### 15.5 版本管理页面增强

- Dialog 改为右侧滑出抽屉组件（Drawer）
- 辅料 Tag 提示、版本号主题色绿色、发布状态 Tag 包裹

#### 15.6 后端新增接口与兼容性修复

- `GET /api/materials/next-code` — 原料编码序列自增
- nutritionController confidence 列自动检测兼容
