...
| 联调测试、集成测试、前后端联调 | `integration` | 联调测试流 |
...
联调：integration-test-generator → integration-test-runner
修复：bug-fixer（合并前后端联调 Bug 统一修复）
```

触发规则：
- 用户说"全栈测试"/"完整测试" → 前端+后端+联调全部走完
- 用户说"测试"（不区分前后端） → 根据上下文推断，无法推断时询问

### 10. 联调测试流 (`integration`)

**完整流程**（用户说"完整联调测试"）：

```
integration-test-generator → integration-test-runner → bug-fixer
```
...
触发规则：
- 用户说"生成联调测试用例"/"集成测试覆盖" → 只触发 integration-test-generator
- 用户说"跑联调测试"/"执行集成测试" → 触发 integration-test-runner
- 用户说"完整联调测试"/"前后端联调" → 三步全走：生成用例 → 执行 → 修复
- 用户说"契约验证"/"接口一致性" → 只触发 integration-test-generator（契约维度）
- 未指定模块 → integration-test-generator 自动扫描并让用户选择
...
| 接口测试用例生成 | api-test-case-generator | — |
| 接口测试执行 | api-test-runner | — |
| 联调测试用例生成 | integration-test-generator | — |
| 联调测试执行 | integration-test-runner | — |
| Bug 修复 | bug-fixer | — |
