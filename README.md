...
- 原料信息 CRUD（新建模式自动生成编码 MAT 序列）
- 唯一编码校验、配方引用检测（防误删）
- **原料列表**：数据看板统计 + 批量操作 + 展开行营养成分 + 活动时间线
- **原料详情**：`header+main` 双区域布局，左栏概况+变更记录，右栏营养网格
- **原料表单**：双栏布局，左侧基础信息+营养分组折叠面板，右侧 AI 营养解析面板
- 能量自动计算（蛋白质×17+脂肪×37+碳水化合物×17），非手动输入
- 原料类型 Tag 区分（药材/辅料）
- 编辑模式 AI 面板遮罩禁用（可见但不可操作）
...
- **智能营养解析**：上传原料营养文档，AI 解析原料名称及每100g含有的4项核心营养素（蛋白质/脂肪/碳水化合物/钠），支持**立即录入**一键创建原料并同步营养数据
...
- 解析结果一键填入配方表单 / 原料立即录入，支持 AI 预填提示和人工校对
...
- **原料管理模块全面重构**：
  - **MaterialList.vue** 全面参照 FormulaList.vue 布局重构：
    - 新增数据看板区域（原料总数/药材数量/辅料数量/平均库存 4 个统计卡片）
    - 批量操作栏（默认隐藏，选中行后显示批量删除按钮）
    - 表格展开行展示完整营养成分数据（6 项核心指标 + 进度条 + NRV 百分比）
    - 活动时间线展示近期原料变更记录（创建/编辑/营养更新）
    - 分页控件内置 pageSize=10，操作列标题与内容完美居中对齐
    - 原料类型 Tag 区分（药材 emerald / 辅料 amber）
  - **MaterialDetail.vue** 参照 FormulaDetail.vue 结构重构：
    - `header + main` 双区域布局，Header 含返回按钮 + 面包屑 + 标题 + 操作按钮
    - Main 区域左栏展示原料概况（基本信息 + 变更记录时间线）
    - Main 区域右栏营养成分网格（每100g含量分析 + 能量计算 + 标准参考 Tag）
    - 能量计算公式可视化：`能量 = 蛋白质×17 + 脂肪×37 + 碳水化合物×17`
    - 营养进度条统一使用主题色绿色 `#10B981`
  - **MaterialForm.vue** 参照 FormulaForm.vue 结构重构：
    - Header 面包屑导航 + 标题「新建/编辑原料」+ 保存/取消按钮
    - Main 区域左侧表单双 Section（基础信息录入 + 营养成分分组折叠面板）
    - Main 区域右侧 **AI 智能营养解析面板**（新建模式可用，编辑模式遮罩禁用）
    - 营养成分按组分类（基础营养/矿物质/维生素/其他），支持 Excel 导入
    - 能量字段改为自动计算值（非输入），基于蛋白质×17+脂肪×37+碳水化合物×17
    - 新建模式原料编码根据规则自动生成填充（MAT001/MAT002...）

- **AI 原料营养解析功能**：
  - 独立于配方解析的新接口 `/api/ai/parse-material-nutrition`
  - 专用提示词模板：解析上传文件中的**原料名称**及每100g含有的**4项核心营养素**（蛋白质/g、脂肪/g、碳水化合物/g、钠/mg）
  - 支持多模态文件上传（Excel/文本/图片），3 个 AI 模型切换（通义千问/智谱GLM/DeepSeek）
  - 解析结果表格展示：原料名称 | 蛋白质 | 脂肪 | 碳水 | 钠 | 匹配状态(已录入/未录入) | **操作**
  - 解析可信度概览：展示各原料名称置信度和整体匹配率
  - Token 消耗量实时显示在结果项中
  - 「重新解析」下拉按钮支持模型选择重新解析

- **立即录入功能**：
  - 未录入原料显示「➕ 录入」按钮（send 图标 + 小尺寸 + 居右对齐）
  - 点击后显示 loading 状态 → 成功显示「✓ 录入成功」→ 失败显示「❌ 重试」
  - 自动完成：生成原料编码(MATxxx)、默认单位(g)、库存(0)、数据来源(AI导入)、备注(文件名)
  - 同时提交原料基础信息 + 营养成分数据到后端数据库
  - 同步更新匹配状态为「已录入」

- **版本管理页面增强**：
  - Dialog 改为从右侧滑出的抽屉组件（Drawer），带入场动画
  - 辅料配方增加 Tag 提示区分
  - 版本号改用主题色绿色，字体 12px，不带括号
  - 原料数量百分比前显示实际重量带单位
  - 发布状态使用 Tag 包裹（草稿 amber / 已发布 emerald）

- **后端新增接口**：
  - `POST /api/ai/parse-material-nutrition` — AI 原料营养解析（独立于配方解析）
  - `GET /api/materials/next-code` — 获取下一个原料编码（MAT 序列自增）
  - nutritionController 兼容性修复：自动检测 `confidence` 列是否存在，兼容未执行迁移的数据库

- **品牌色与样式统一**：
  - 绿清新品牌色统一为 RGB(16, 185, 129) / `#10B981`
  - 所有主题色相关 UI 组件（Tag/进度条/按钮/图标）统一使用新色号

#### 影响范围

- **前端视图**: MaterialForm.vue, MaterialList.vue, MaterialDetail.vue, VersionList.vue, FormulaForm.vue
- **前端 API**: ai.ts（新增 material parse 接口）, material.ts（新增 nextCode 接口）, nutrition.ts
- **前端 Store**: ai.ts（新增 materialParseResult 状态管理）
- **后端控制器**: aiController.ts（新增 parseMaterialNutrition）, materialController.ts（新增 getNextCode）, nutritionController.ts（confidence 兼容）
- **后端路由**: ai.ts, materials.ts
